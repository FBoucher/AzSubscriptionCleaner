...
foreach ($r in $expResources) {
    ...
    Remove-AzResource -ResourceId $r.id -Force -WhatIf #Remove WhatIf when in Production
}
...
foreach($resourceGroup in $rgs){
    ...
    if($count -eq 0){
        ...
        Remove-AzResourceGroup -Name $name -Force -WhatIf #Remove WhatIf when in Production
    }
}
